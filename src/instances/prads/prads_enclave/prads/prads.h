...
uint32_t hton_long(uint32_t n);
uint32_t ntoh_long(uint32_t n);
#define IP6_V(header)                     (hton_long(header->vcl) >> 28)
//#define IP6_TC(ip6_header)                (((hton_long(ip6_header)->vcl) & 0x0FF00000) >> 20)
#define IP6_TC(ip6_header)                ((hton_long((ip6_header)->vcl) & 0x0FF00000) >> 20)
#define IP6_FL(ip6_header)                (hton_long((ip6_header)->vcl) & 0x000FFFFF)
...
#define GRE_GET_PROTO(r)          ntoh_short(r->ether_type)
...
//    /* For tracing LRU flow cache */
//    struct   _connection *cache_prev;
//    struct   _connection *cache_next;
//    /* Track flow cache allocation */
//#define FLOW_CACLE_NOT_ALLOCATED -1
//    int      flow_cache_idx;
    /* Store bucket hash to save lookup time */
    //uint32_t hash;
    /* placeholder for maintaining 1K per-flow states */
    //state_entry *hosting_entry;
...
typedef struct _packetinfo {
    ...
    const pcap_pkthdr *pheader; /* Libpcap packet header struct pointer */
...
uint16_t ntoh_short(uint16_t n);
#define PI_TCP_SP(pi) ( ntoh_short((pi)->tcph->src_port) )
#define PI_TCP_DP(pi) ( ntoh_short((pi)->tcph->dst_port) )
// and more to come
...
//int sgbox_proto_unsupport(packetinfo *pi);
