...
fp_entry *find_match(
   ...
   // uint32_t dst, // pi
   // uint16_t sp,  // ntoh_short(pi->tcph->src_port)
   // uint16_t dp, 
   // uint8_t ocnt, // optcnt
   // uint8_t* op,  // opt
   // uint16_t mss, 
...
fp_entry *fp_tcp(packetinfo *pi, uint8_t ftype)
{
    ...
    switch(pi->af){
        case AF_INET6:
            opt_ptr = (uint8_t *) pi->ip6 + IP6_HEADER_LEN + ntoh_short(pi->ip6->len); //*
            if (end_ptr > opt_ptr)
                end_ptr = opt_ptr;
            ...
            e.size = (ftype == TF_ACK) ? 0 : ntoh_short(pi->ip6->len);
            e.df = 1; // for now
            if (!IP6_FL(pi->ip6)) //*
                e.quirks |= QUIRK_ZEROID;
            break;
        case AF_INET:
            opt_ptr = (uint8_t *) pi->ip4 + ntoh_short(pi->ip4->ip_len);
            if (end_ptr > opt_ptr)
                end_ptr = opt_ptr;
            ...
            e.ttl = pi->ip4->ip_ttl;
            e.size = (ftype == TF_ACK) ? 0 : ntoh_short(pi->ip4->ip_len);
            e.df = (ntoh_short(pi->ip4->ip_off) & IP_DF) != 0;
            if (!pi->ip4->ip_id)
                e.quirks |= QUIRK_ZEROID;
            break;
    ...
    e.wsize = ntoh_short(pi->tcph->t_win);
...
/*
printf("hop:%u, len:%u, ver:%u, class:%u, label:%u|mss:%u, win:%u\n",ip6->hop_lmt,open_mode ? 0 : ntoh_short(ip6->len),
                                                     IP6_V(ip6),ntoh_short(IP6_TC(ip6)),
                                                     ntoh_short(IP6_FL(ip6)),
                                                     mss_val, ntoh_short(tcph->t_win));
*/


//     /* sp */    ntoh_short(tcph->sport),
//     /* dp */    ntoh_short(tcph->dport),
//     /* ocnt */  ocnt,
//     /* op */    op,
//     /* mss */   mss_val,
//     /* wsc */   wsc_val,
