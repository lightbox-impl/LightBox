...
void get_clock(timeval_t* ts)
{
    ...
    /* else */
    /* *ts = 0; */
    memcpy(ts, &time_now, sizeof(timeval_t));
}
...
void write_pkt(const uint8_t* pkt, int pkt_size, timeval_t ts)
{
...
void read_pkt(uint8_t* pkt, int* size, timeval_t* ts)
{
    ...
    *size = in_rbuf[read].size;
    /* *ts = in_rbuf[read].ts; */
    memcpy(ts, &(in_rbuf[read].ts), sizeof(ts));
    read = NEXT(read);
...
void write_pkt(const uint8_t* pkt, int pkt_size, timeval_t ts)
{
    ...
    /* in_rbuf[write].ts = ts; */
    memcpy(&(in_rbuf[write].ts), &ts, sizeof(ts));
...
void read_pkt(uint8_t* pkt, int* size, timeval_t* ts)
{
    ...
    *size = in_rbuf[read].size;
    /* *ts = in_rbuf[read].ts; */
    memcpy(ts, &(in_rbuf[read].ts), sizeof(timeval_t));
    read = NEXT(read);
...
void write_pkt(const uint8_t* pkt, int pkt_size, timeval_t ts)
{
    ...
    in_rbuf[write].size = pkt_size;
    /* in_rbuf[write].ts = ts; */
    memcpy(&(in_rbuf[write].ts), &ts, sizeof(timeval_t));
    write = NEXT(write);
