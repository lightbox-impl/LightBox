...
#define TRY_ASSIGN_NUM(name, base, item, value)                           \
	((strncmp(#name, item, strlen(#name)) == 0) &&                    \
	 (!isalnum(item[strlen(#name)])) &&                               \
	 (sscanf(value,                                                   \
		 (sizeof((base)->name) == sizeof(char))                   \
		     ? "%hhi"                                             \
		     : (sizeof((base)->name) == sizeof(short))            \
			   ? "%hi"                                        \
			   : (sizeof((base)->name) == sizeof(int))        \
				 ? "%i"                                   \
				 : (sizeof((base)->name) == sizeof(long)) \
				       ? "%li"                            \
				       : (sizeof((base)->name) ==         \
					  sizeof(long long))              \
					     ? "%lli"                     \
...
#define LINE_FOREACH(line, llen, buf, blen)                                    \
	for (line = buf,                                                       \
	    llen = ((strchr(line, '\n') == NULL) ? (buf + blen - line)         \
						 : strchr(line, '\n') - line); \
	     line + llen < buf + blen; line += llen + 1,                       \
	    llen = ((strchr(line, '\n') == NULL) ? (buf + blen - line)         \
						 : strchr(line, '\n') - line)) \
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
static int DetectWord(char *buf, int len, char **word, int *wlen) {
	int i;
	for (i = 0; i < len; i++) {
	...
	for (*wlen = 0; *wlen < len; (*wlen)++) {
		if (isalnum((*word)[*wlen]) || (*word)[*wlen] == '_') continue;
...
/*----------------------------------------------------------------------------*/
static int ReadItemValue(char *line, int llen, char *item, int ilen,
			 char *value, int vlen) {
	const char *end = &line[llen];
	char *word = NULL;
	int wlen = 0;

	if (DetectWord(line, llen, &word, &wlen) < 0 || wlen > ilen) return -1;

	line = word + wlen;

	/* skip space */
	while (line < end && isspace(*line)) line++;

	if (*(line++) != '=') return -1;

	while (line < end && isspace(*line)) line++;

	if (end - line > vlen) return -1;
...
/*----------------------------------------------------------------------------*/
static void FeedAppConfLine(struct conf_block *blk, char *line, int len) {
	...
	if (TRY_ASSIGN_STR(type, conf, item, value))
		...
		StrToArgs(conf->run, &conf->app_argc, conf->app_argv,
			  MOS_APP_ARGC);
#if 0
...
/*----------------------------------------------------------------------------*/
static void FeedMosConfLine(struct conf_block *blk, char *line, int len) {
...
/*----------------------------------------------------------------------------*/
static void FeedNetdevConfLine(struct conf_block *blk, char *line, int len) {
	struct netdev_conf *const conf = (struct netdev_conf *)blk->conf;

	printf("read netDev table in sgx config.c:%d", __LINE__);
	...
	//	int wlen;
	//
	//	if (DetectWord(line, len, &word, &wlen) < 0 || wlen > WORD_LEN
	//|| wlen <= 0) 		return;
	//
	//	line = word + wlen;
	//
	...
	//		return;
	//
	//	struct netdev_entry *ent = calloc(1, sizeof(struct
	//netdev_entry)); 	if (!ent) { 		TRACE_ERROR("Could not allocate memory for
	//netdev_entry!\n"); 		exit(EXIT_FAILURE);
	//	}
	//
	...
	//		struct in_addr sin = ((struct sockaddr_in
	//*)&ent->ifr.ifr_addr)->sin_addr; 		ent->ip_addr = *(uint32_t *)&sin;
	//	}
	//
	//	/* Net MASK */
	//	if (ioctl(sock, SIOCGIFNETMASK, &ent->ifr) == 0) {
	//		struct in_addr sin = ((struct sockaddr_in
	//*)&ent->ifr.ifr_addr)->sin_addr; 		ent->netmask = *(uint32_t *)&sin;
	//	}
	//
	//#ifdef DARWIN
...
/*----------------------------------------------------------------------------*/
static void FeedArpConfLine(struct conf_block *blk, char *line, int len) {
	...
	//			address, &prefix, &haddr[0], &haddr[1],
	//&haddr[2], 			&haddr[3], &haddr[4], &haddr[5]) != 8) 	return;
...
/*----------------------------------------------------------------------------*/
static void FeedRouteConfLine(struct conf_block *blk, char *line, int len) {
...
/*----------------------------------------------------------------------------*/
static void FeedNICFwdConfLine(struct conf_block *blk, char *line, int len) {
	struct nic_forward_conf *const conf =
	    (struct nic_forward_conf *)blk->conf;
	...
	struct nic_forward_entry *ent =
	    calloc(1, sizeof(struct nic_forward_entry));
	if (!ent) {
		TRACE_ERROR(
		    "Could not allocate memory for nic forward entry!\n");
		exit(EXIT_FAILURE);
	}
...
/*----------------------------------------------------------------------------*/
static void MosConfAddChild(struct conf_block *blk, struct conf_block *child) {
	struct mos_conf *const conf = (struct mos_conf *)blk->conf;
	...
	if (strcmp(child->name, NETDEV_BLOCK_NAME) == 0) {
		...
		conf->nic_forward_table =
		    (struct nic_forward_conf *)child->conf;
	} else
		return;
}
/*----------------------------------------------------------------------------*/
static int AppConfIsValid(struct conf_block *blk) {
	struct app_conf *const conf = (struct app_conf *)blk->conf;

	if (conf->app_argc <= 0) return 0;

	return 1;
}
/*----------------------------------------------------------------------------*/
static int MosConfIsValid(struct conf_block *blk) { return 1; }
/*----------------------------------------------------------------------------*/
static int NetdevConfIsValid(struct conf_block *blk) { return 1; }
/*----------------------------------------------------------------------------*/
static int ArpConfIsValid(struct conf_block *blk) { return 1; }
/*----------------------------------------------------------------------------*/
static int RouteConfIsValid(struct conf_block *blk) { return 1; }
/*----------------------------------------------------------------------------*/
static int NICFwdConfIsValid(struct conf_block *blk) { return 1; }
/*----------------------------------------------------------------------------*/
static void NetdevConfPrint(struct conf_block *blk) {
	struct netdev_conf *const conf = (struct netdev_conf *)blk->conf;
	...
	TAILQ_FOREACH(walk, &conf->list, link) {
		printf(
		    " | %s(idx: %d, HADDR: "
		    "%02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX) maps to CPU "
		    "0x%016lX\n",
		    walk->dev_name, walk->ifindex, walk->haddr[0],
		    walk->haddr[1], walk->haddr[2], walk->haddr[3],
		    walk->haddr[4], walk->haddr[5], walk->cpu_mask);
	}
	printf(" |\n");
}
/*----------------------------------------------------------------------------*/
static void ArpConfPrint(struct conf_block *blk) {
	struct arp_conf *const conf = (struct arp_conf *)blk->conf;
	...
	TAILQ_FOREACH(walk, &conf->list, link) {
		printf(
		    " | IP: 0x%08X, NETMASK: 0x%08X, "
		    "HADDR: %02hhX:%02hhX:%02hhX:%02hhX:%02hhX:%02hhX\n",
		    ntohl(walk->ip), ntohl(walk->mask), walk->haddr[0],
		    walk->haddr[1], walk->haddr[2], walk->haddr[3],
		    walk->haddr[4], walk->haddr[5]);
	}
	printf(" |\n");
}
/*----------------------------------------------------------------------------*/
static void RouteConfPrint(struct conf_block *blk) {
	struct route_conf *const conf = (struct route_conf *)blk->conf;
	...
	TAILQ_FOREACH(walk, &conf->list, link) {
		printf(
		    " | IP: 0x%08X, NETMASK: 0x%08X, INTERFACE: %s(idx: %d)\n",
		    ntohl(walk->ip), ntohl(walk->mask), walk->dev_name,
		    walk->nif);
	}
	printf(" |\n");
}
/*----------------------------------------------------------------------------*/
static void NICFwdConfPrint(struct conf_block *blk) {
	int i;
	struct nic_forward_conf *const conf =
	    (struct nic_forward_conf *)blk->conf;

	printf(
	    " +===== NIC Forwarding table configuration (%d entries) =====\n",
	    conf->num);

	struct nic_forward_entry *walk;
	TAILQ_FOREACH(walk, &conf->list, link) {
		printf(" | NIC Forwarding Entry: %s <---> %s", walk->nif_in,
		       walk->nif_out);
	}
	printf(" |\n");
...
/*----------------------------------------------------------------------------*/
static void AppConfPrint(struct conf_block *blk) {
	struct app_conf *const conf = (struct app_conf *)blk->conf;
...
/*----------------------------------------------------------------------------*/
static void MosConfPrint(struct conf_block *blk) {
	struct mos_conf *const conf = (struct mos_conf *)blk->conf;
	...
	printf("|\n");
	if (conf->netdev) conf->netdev->print(conf->netdev);
	if (conf->arp) conf->arp->print(conf->arp);
	if (conf->route) conf->route->print(conf->route);
	if (conf->nic_forward) conf->nic_forward->print(conf->nic_forward);
	printf("\n");
}
/*----------------------------------------------------------------------------*/
static void InitAppBlock(struct config *config, struct conf_block *blk) {
	assert(blk);
...
/*----------------------------------------------------------------------------*/
static void InitMosBlock(struct config *config, struct conf_block *blk) {
	assert(blk);
...
/*----------------------------------------------------------------------------*/
static void InitNetdevBlock(struct config *config, struct conf_block *blk) {
	assert(blk);
...
/*----------------------------------------------------------------------------*/
static void FetchARPKernelEntries(struct arp_conf *const config) {
	config->num = 0;
	...
	//
	//			num = sscanf(line, "%s 0x%x 0x%x %100s %100s
	//%100s\n", 				     ip, &type, &flags, hwa, mask, dev); 			if (num < 6) 				break;
	//
	//			/* if the user specified device differs, skip it
	//*/ 			if (strncmp(dev, DPDK_PREFIX, DPDK_PREFIX_LEN)) 				continue;
	//
	//			/* if the entry has not expired/tagged for removal
	//then... */ 			if (flags != 0x00) {
	//				/* add the new arp entry in MOS database
	//*/ 				struct _arp_entry *ent = calloc(1, sizeof(struct _arp_entry)); 				if
	//(!ent) { 					TRACE_ERROR("Can't allocate memory for arp_entry\n");
	//					exit(EXIT_FAILURE);
	//				}
	//				uint8_t haddr[ETH_ALEN] = {0};
	//				if (sscanf(hwa,
	//"%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", 					   &haddr[0], &haddr[1], &haddr[2],
	//					   &haddr[3], &haddr[4], &haddr[5]) !=
	//6) { 					TRACE_ERROR("Error reading the ARP entry\n"); 					exit(EXIT_FAILURE);
	//				}
	//				ent->ip = inet_addr(ip);
	//				ent->prefix = 32;
	//				ent->mask = htonl((ent->prefix == 0) ? 0 :
	//((-1) << (32 - ent->prefix))); 				ent->masked_ip = ent->mask & ent->ip;
	//				memcpy(ent->haddr, haddr, ETH_ALEN);
	//				TAILQ_INSERT_TAIL(&config->list, ent,
	//link); 				config->ent[config->num] = ent; 				config->num++;
	//			}
	//		}
	//	}
	//
...
/*----------------------------------------------------------------------------*/
static void FetchRouteKernelEntries(struct route_conf *const config) {
	config->num = 0;
	...
	//		for (; fgets(line, sizeof(line), fp);) {
	//
	//			num = sscanf(line, "%s %08X %08X %d %d %d %d %08X %s
	//%s %s\n", 				     dev, 				     &dest, 				     &gate, 				     &flags, &cnt, &use, &metric, 				     &mask, 				     mtu,
	//win, irtt);
	//
	//			if (num < 11)
	//				break;
	//#if 0
	//			/* if the user specified device differs, skip it
	//*/ 			if (strncmp(dev, DPDK_PREFIX, DPDK_PREFIX_LEN)) 				continue; #endif
	//			struct route_entry *ent = calloc(1, sizeof(struct
	//route_entry)); 			if (!ent) { 				TRACE_ERROR("Could not allocate memory for
	//route_entry!\n"); 				exit(EXIT_FAILURE);
	//			}
	//
	//			ent->ip = dest;
	//
	//			/* __builtin_clz() returns undefined output with
	//zero */ 			if (mask == 0) 				ent->prefix = 0; 			else 				ent->prefix = 32 -
	//__builtin_clz(mask); 			ent->mask = mask; 			ent->masked_ip = ent->mask &
	//ent->ip; 			strcpy(ent->dev_name, dev); 			TAILQ_INSERT_TAIL(&config->list,
	//ent, link); 			config->ent[config->num] = ent; 			config->num++;
	//		}
	//	}
	//
	//	fclose(fp);
...
/*----------------------------------------------------------------------------*/
static void InitArpBlock(struct config *config, struct conf_block *blk) {
	assert(blk);
...
/*----------------------------------------------------------------------------*/
static void InitRouteBlock(struct config *config, struct conf_block *blk) {
	assert(blk);
...
/*----------------------------------------------------------------------------*/
static void InitNICForwardBlock(struct config *config, struct conf_block *blk) {
	int i;
	assert(blk);
	...
	struct nic_forward_conf *conf =
	    calloc(1, sizeof(struct nic_forward_conf));
	if (conf == NULL) {
		TRACE_ERROR(
		    "Could not allocate memory for nic_forward_conf!\n");
		exit(EXIT_FAILURE);
	}
	for (i = 0; i < MAX_FORWARD_ENTRY; i++) conf->nic_fwd_table[i] = -1;
...
/*----------------------------------------------------------------------------*/
void PrintConf(struct config *conf) {
...
/*----------------------------------------------------------------------------*/
static void CheckConfValidity(struct config *conf) {
	struct conf_block *walk;
	TAILQ_FOREACH(walk, &conf->app_blkh, link) {
		if (!walk->isvalid || !walk->isvalid(walk)) goto __error;
	}
	...
	TAILQ_FOREACH(walk, &conf->mos_blkh, link) {
		...
		if (!child->isvalid || !child->isvalid(child)) goto __error;
...
__error:
	...
	if (walk && walk->print) walk->print(walk);
	exit(0);
}
/*----------------------------------------------------------------------------*/
// static char *
...
//}
/*----------------------------------------------------------------------------*/
static char *PreprocessConf(char *raw) {
	char *line;
	int llen;
	...
	LINE_FOREACH(line, llen, raw, len) {
		int i, iscomment = 0;
		for (i = 0; i < llen; i++) {
			if (!iscomment && line[i] == '#') iscomment = 1;
			if (iscomment) line[i] = ' ';
		}
	}
	return raw;
...
/*----------------------------------------------------------------------------*/
static void InitConfig(struct config *config) {
	int i;
...
/*----------------------------------------------------------------------------*/
static struct conf_block *AllocateBlock(char *name, int len) {
	struct conf_block *walk, *tmp;

	for (walk = TAILQ_FIRST(&g_free_blkh); walk != NULL; walk = tmp) {
		tmp = TAILQ_NEXT(walk, link);
		if (len == strlen(walk->name) &&
		    strncmp(walk->name, name, len) == 0) {
...
/*----------------------------------------------------------------------------*/
struct conf_block *DetectBlock(struct conf_block *blk, char *buf, int len) {
	int depth = 0;
	...
	if (DetectWord(buf, len, &blkname, &blknamelen) < 0 || blkname != buf)
	...
	/* buf must be '{' */
	if (buf >= end || *buf != '{') return NULL;

	buf++;					  /* skip '{' */
	while (buf < end && isspace(*buf)) buf++; /* skip space */
	...
	if (depth != 0) /* Failed to find the end of parenthesis */
		return NULL;

	if (!(nblk = AllocateBlock(blkname, blknamelen))) return NULL;
...
/*----------------------------------------------------------------------------*/
static void ParseBlock(struct conf_block *blk) {
	char *line;
	int llen;
	...
	LINE_FOREACH(line, llen, blk->buf, blk->len) {
		...
		if ((nblk = DetectBlock(blk, line,
					blk->len - (line - blk->buf)))) {
			ParseBlock(nblk);
...
/*----------------------------------------------------------------------------*/
void PatchCONFIG(struct config *config) {
	int i, count;
	char *word, *str, *end;
	int wlen;
	uint64_t value;
	...
	if (count > num_cpus) {
		TRACE_ERROR(
		    "CPU mask (%016lX) exceeds the number of CPU cores (%d)\n",
		    g_config.mos->cpu_mask, num_cpus);
		exit(-1);
	}
	if (count > MAX_CPUS) {
		TRACE_ERROR(
		    "mOS does not support more than %d CPU cores for now\n",
		    MAX_CPUS);
		exit(-1);
	}
	...
	TAILQ_FOREACH(bwalk, &g_config.app_blkh, link) {
		...
		g_config.mos->forward =
		    g_config.mos->forward && app_conf->ip_forward;
	...
	while (DetectWord(str, end - str, &word, &wlen) == 0) {
		for (i = 0; i < g_config.mos->netdev_table->num; i++) {
			if (strncmp(
				...
				word, wlen) == 0) {
				g_config.mos->netdev_table->ent[i]->stat_print =
				    TRUE;
			}
		}
		str = word + wlen;
...
/*----------------------------------------------------------------------------*/
int LoadConfigurationUpperHalf(const char *fname) {
	ReadConf(fname, &g_config);
	// char *line;
	// int llen;
	...
	//	if ((nblk = DetectBlock(NULL, line, len - (line -
	//preprocessed)))) { 		ParseBlock(nblk);
...
/*----------------------------------------------------------------------------*/
void LoadConfigurationLowerHalf(void) {
	...
	TAILQ_FOREACH(rwalk, &route_conf->list, link) {
		TAILQ_FOREACH(nwalk, &netdev_conf->list, link) {
			if (!strcmp(nwalk->dev_name, rwalk->dev_name)) break;
		}
		if (!nwalk) continue;
		if (nwalk->ifindex < 0 &&
		    (nwalk->ifindex =
			 current_iomodule_func->get_nif(&nwalk->ifr)) < 0) {
...
int FetchEndianType() {
	...
	if (current_iomodule_func == &dpdk_module_func) {
		...
		if (!strcmp(*argp, "net_i40e")) return 1;
