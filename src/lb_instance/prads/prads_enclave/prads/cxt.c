...
int flow_count;
//connection *bucket[BUCKET_SIZE] = { NULL };

#if SGBOX==1 
#include "../sg-box/secure_ferry_t.h"
#include "../sg-box/state_orchestrator_t.h"
extern state_entry bundle_buffer_state[];
extern flow_id bundle_buffer_id[];
extern int bundle_is_client[];

#define STATE_ENTRY_CXT(entry) (connection *)(&entry->state)
#define check_cx_to_end(cxt) \
        (cxt->s_tcpFlags & TF_FIN && cxt->d_tcpFlags & TF_FIN) || \
        (cxt->s_tcpFlags & TF_RST || cxt->d_tcpFlags & TF_RST)
#if NO_BUNDLE==1
connection *bucket[BUCKET_SIZE] = { NULL };
...
//#if SGBOX==1
#if LightBox==1
// initialize newly tracked cxt
void new_cxt_init(connection *cxt, packetinfo *pi) 
{
    struct in6_addr ips;
    struct in6_addr ipd;
    cxtrackerid++;
    ...
    cxt->proto = pi->proto;
    cxt->hw_proto = ntoh_short(pi->eth_type);
    ...
    // to avoid the padding being optimized out by compiler
    memcpy(cxt->garbage, &cxt->last_pkt_time, 4);
}
#else
...
#if SGBOX==1
#if NO_BUNDLE==1
extern int _miss_count;
int cx_track(packetinfo *pi) {
    flow_id pending;
    pending.src_ip = pi->ip4->ip_src;
    pending.dst_ip = pi->ip4->ip_dst;
    pending.src_port = pi->s_port;
    pending.dst_port = pi->d_port;
    pending.proto = pi->proto;

    int is_client;
    state_entry *entry = state_cache_lookup(&pending, &is_client);
    // cache hit
    if (entry) {
        connection *cxt = STATE_ENTRY_CXT(entry);

        entry->last_pkt_time = pi->pheader->ts.tv_sec;
        entry->to_end = check_cx_to_end(cxt);

        raise_to_front(entry);
    ...
    // cache miss
        ...
        // Case 1: state_cache is still cold, so the flow must be a fresh one
        //         and we don't need to lookup the state_repo.
        //         This only happends in the very beginning.
        if (unlikely(state_cache_cold())) {
            entry = state_cache_cold_allocate();

            // init entry
            entry->id = pending;
            entry->hash = LKUP_HASH((&pending));
            entry->last_pkt_time = pi->pheader->ts.tv_sec;
            entry->to_end = 0;
            new_cxt_init(STATE_ENTRY_CXT(entry), pi);

            push_to_front(entry);

            return 1;
        }
        // Case 2: flow cache is alreay full, so probably the missed flow has been
        //         evicted from state_cache to state_repo, or it is a new one.
        //         We only know which case it is until secure_swap returns
        else {
            ++_miss_count;
            // Kick out the LRU entry, i.e. the rear
            state_entry *entry = drop_from_rear();
            int evicted_cache_idx = entry->state_cache_idx;

            encrypt_flowid(&pending);

            int is_client;
            secure_swap_single(entry, &pending, &is_client);

            if (is_entry_valid(entry)) {
                entry->state_cache_idx = evicted_cache_idx;
                ...
                if (is_client == 1) {
                    ...
                    eprintf("This is not possible dude!\n");
                    return 1;
                }
            }
            else {
...
#else
extern packetinfo ferry_pktinto[];
extern int        ferry_pkt_status[];
extern int        ferry_idx;
...
int cx_track(packetinfo *pi) {
    ...
    // cache miss
        ...
        //         We only know which case it is until secure_swap returns
            ...
            int bundled_idx = lookup_bundle_buffer(&pending);
            // Case 2.a: the packet has been bundled in the same ferry
            if (bundled_idx != ferry_pkt_bundle_not_found) {
                ferry_pkt_status[ferry_idx] = bundled_idx;
            }
            // Case 2.b: the packet has not yet been bundled
            else {
                // Kick out the LRU entry, i.e. the rear
                state_entry *evicted = drop_from_rear();

                encrypt_flowid(&pending);

                ferry_pkt_status[ferry_idx] = bundle_state(evicted, &pending);
            }
            // suspend the processing after bundling, wait for entire ferry ready
            return 1;
        }
    }
}
...
int continue_track() {
    packetinfo *pi = &ferry_pktinto[ferry_idx];
    int bundle_id = ferry_pkt_status[ferry_idx];

    int is_client = -1;
    // Try re-lookup the state_cache, in case multiple packets belong to 
    // the same flow in a ferry, which is critical to correctness and performance
    state_entry *entry = state_cache_lookup(&bundle_buffer_id[bundle_id], &is_client);
    if (entry) {
        connection *cxt = STATE_ENTRY_CXT(entry);
        ...
        // successfully swapped in entry from outside
        if (is_entry_valid(&bundle_buffer_state[bundle_id])) {
            // reuse evicted entry
            entry = state_cache_reallocate(bundle_id);
            ...
            if (bundle_is_client[bundle_id] == 1) {
                return cxt_update_client(cxt, pi);
            }
            else if (bundle_is_client[bundle_id] == 0) {
                return cxt_update_server(cxt, pi);
            }
            else {
                eprintf("This is not possible dude!\n");
        ...
        // No existing entry found even outside, hence new flow.
        else {
            entry = state_cache_allocate(bundle_id);

            entry->id = bundle_buffer_id[bundle_id];
            entry->hash = LKUP_HASH((&entry->id));
            entry->last_pkt_time = pi->pheader->ts.tv_sec;
            entry->to_end = 0;
            new_cxt_init(STATE_ENTRY_CXT(entry), pi);
...
#else
#include "../sg-box/lb_state_t.h"
extern int stats_cache_hit;
extern int stats_store_hit;
extern int stats_miss;
#define check_cx_to_end(cxt) \
        (cxt->s_tcpFlags & TF_FIN && cxt->d_tcpFlags & TF_FIN) || \
        (cxt->s_tcpFlags & TF_RST || cxt->d_tcpFlags & TF_RST)
int cx_track(packetinfo *pi) {
	static fid_t pending;
	pending.src_ip = pi->ip4->ip_src;
	pending.dst_ip = pi->ip4->ip_dst;
	pending.src_port = pi->s_port;
	pending.dst_port = pi->d_port;
	...
	state_entry_t *raw_state;
	flow_tracking_status rlt = flow_tracking(&pending, &raw_state, pi->pheader->ts.tv_sec,
											 0); // test idx = 0
	connection *cxt = (connection *)&raw_state->state;
	switch (rlt) {
		// new flow
	case ft_miss:
		new_cxt_init(cxt, pi);
	...
	//if (cxt->last_pkt_time < 0) {
	//	eprintf("why? %d\n", rlt);
	//	abort();
	//}

	/* pre-checking of flow termination */
	raw_state->to_end = check_cx_to_end(cxt);

	if(raw_state->is_client == 1)
		return cxt_update_client(cxt, pi);
	else if(raw_state->is_client == 0)
		return cxt_update_server(cxt, pi);
	else{
		eprintf("error : %d\n", raw_state->is_client);
		abort();
...
*/
void end_sessions(uint64_t wall_time)
{
    connection *cxt;
    int iter;
    int cxstatus = CX_NONE;
    time_t check_time = wall_time;
...
#else
// ad-hoc implementation without changing original logic
#include "../prads_t.h"
extern struct cuckoo_hash store_lkup_table;
void end_sessions(uint64_t check_time)
{
	int cxstatus = CX_NONE;
	struct cuckoo_hash_item *it;
	for (cuckoo_hash_each(it, &store_lkup_table))
	{
		state_entry_t *raw_state = it->value;
		connection *cxt = (connection *)&raw_state->state;
		// use the time stored in state entry
		// note that cxt is encrypted outside enclave
		time_t last_pkt_time = raw_state->last_access_time;
		/* TCP */
		if (cxt->proto == IP_PROTO_TCP) {
			/* * FIN from both sides */
			if (cxt->s_tcpFlags & TF_FIN && cxt->d_tcpFlags & TF_FIN
				&& (check_time - last_pkt_time) > 5) {
				eprintf("tcp fin\n");
				cxstatus = CX_ENDED;
			} /* * RST from either side */
			else if ((cxt->s_tcpFlags & TF_RST
			...
			else if ((check_time - last_pkt_time) > TCP_TIMEOUT) {
				eprintf("tcp exp\n");
				cxstatus = CX_EXPIRE;
			}
		}
		...
		/* ICMP */
			...
			|| cxt->proto == IP6_PROTO_ICMP) {
			if ((check_time - last_pkt_time) > 60) {
				cxstatus = CX_EXPIRE;
				eprintf("icmp exp\n");
			}
		}
		...
		// A quick fix for above buggy logic
		if (raw_state->to_end && (check_time - last_pkt_time) > 5) {
			cxstatus = CX_ENDED;
		}
		...
		if (cxstatus == CX_ENDED || cxstatus == CX_EXPIRE) {
			...
			//eprintf("remove state!\n");
			cuckoo_hash_remove(&store_lkup_table, raw_state->lkup);
			ocall_state_store_free(raw_state);
		}
	}
}
#endif
