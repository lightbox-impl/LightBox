...
mtcp_peek(mctx_t mctx, int msock, int side, char *buf, size_t len) 
{
	...
	if (cur_stream->rcvvar != NULL && 
		...
		rc = rcvbuf->cur_recv_size;
		if (len < rc)
		{
			TRACE_ERROR("Peek buffer is too small, need %d!\n", rc);
			return -1;
		}
		memcpy(buf, rcvbuf->recv_buffer, rc);
		rcvbuf->cur_recv_size = 0;
#else
		...
		*poff += rc;
		...
		//TRACE_ERROR("Stream hasn't yet been initialized!\n");
