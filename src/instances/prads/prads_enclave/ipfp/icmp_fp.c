              ...
              const uint8_t * end_ptr)
{
     ...
     */
    opt_ptr = (uint8_t *) ip4 + ntoh_short(ip4->ip_len);
    if (end_ptr > opt_ptr)
        end_ptr = opt_ptr;
    ...
    gen_fp_icmp(icpmh->type, icpmh->code, ip4->ip_ttl,
                (ntoh_short(ip4->ip_off) & IP_DF) != 0, olen,
                 ntoh_short(ip4->ip_len), idata, ip4->ip_off, ip4->ip_tos,
                quirks,
                //ip_src, AF_INET);
                pi);
              ...
              const uint8_t * end_ptr)
{
     ...
     * or such), truncate the package. 
     */
    opt_ptr = (uint8_t *) ip6 + ntoh_short(ip6->len);
    if (end_ptr > opt_ptr)
        end_ptr = opt_ptr;
    ...
    if ((uint8_t *) opt_ptr + ntoh_short(ip6->len) < end_ptr) {
        quirks |= QUIRK_DATA;
...
//                                               ntoh_short(ip6->len),IP6_V(ip6),IP6_TC(ip6),
//                                               IP6_FL(ip6));
/*
   gen_fp_icmp(icpmh->type,icpmh->code,ip4->ip_ttl,(ntoh_short(ip4->ip_off) & IP_DF) != 0,olen,
                     ntoh_short(ip4->ip_len),idata,ip4->ip_off,ip4->ip_tos,quirks, ip_src, AF_INET);
*/
}
