...
#include "include/bpf/sfbpf_dlt.h"
...
int ocall_sfbpf_filter(const char *pc, const char *p, int wirelen, int buflen)
{
	return sfbpf_filter((const struct sfbpf_insn *)pc, (const unsigned char*)p, wirelen, buflen);
}

void ocall_sfbpf_freecode(char *program)
{
...
extern const char* pcap_file_name;
/*----------------------------------------------------------------------------*/
void ocall_pcap_init(char* filename)
{
	...
	if (strstr(filename, ".pcap"))
	{
		...
		g_pcap_handle = pcap_open_offline(pcap_file_name, errbuf);
		...
		//if (pcap_compile(g_pcap_handle, &fp, filter_exp, 0, net) == -1) {
	...
	{
		printf("Open pcap interface %s.\n", pcap_file_name);

		g_pcap_handle = pcap_create(pcap_file_name, errbuf);
		if (!g_pcap_handle) {
			printf("Interface '%s' not found\n", pcap_file_name);
			exit(EXIT_FAILURE);
		}

		if (pcap_set_buffer_size(g_pcap_handle, PCAP_BUFFER_SIZE) != 0) {
			printf("Can't increase buffer size to %u bytes\n",
				PCAP_BUFFER_SIZE);
		}

		if (pcap_activate(g_pcap_handle) < 0) {
			printf("Failed to activate pcap handle!\n");
			pcap_perror(g_pcap_handle, "");
			exit(EXIT_FAILURE);
		}
		...
		{
			struct bpf_program fp;		/* The compiled filter */
			char filter_exp[] = "tcp dst port 12345";	/* The filter expression */
			bpf_u_int32 mask;		/* Our netmask */
			bpf_u_int32 net;		/* Our IP */
...
void ocall_pcap_next(char** pkt, char* pkthdr)
{
	if (!g_pcap_handle)
	{
		ocall_pcap_init("hi.");
	...
	const uint8_t *packet = pcap_next(g_pcap_handle, &head);

	if (!packet || head.caplen!=head.len ||head.caplen>1514)
	{
		//char* errInfo= pcap_geterr(g_pcap_handle);
		//printf("Couldn't get pkt : %s\n", errInfo? errInfo:"eof");
		printf("Eof, exit.\n");
		memset(&head, 0, sizeof(head));
		memcpy(pkthdr, &head, sizeof(head));
		return;
	}
...
#define TEST_ITVL 200000
	...
	//if (*pkt&&no++%500==0)
	//{
	//	printf("ocall read no.500 pkt size is %d\n", head.len);
	//}
	memcpy(pkthdr, &head, sizeof(head));
}
...
void ocall_load_config(const char* filename, char* configs)
{
	//int8_t end_app_exists = 0;
	//int8_t mon_app_exists = 0;
	//addr_pool_t ap[ETH_NUM] = { NULL };
	...
	LoadConfigurationUpperHalf(filename);

	printf("read config complete.\n ");

	memcpy(configs, &g_config, sizeof(g_config));
}
