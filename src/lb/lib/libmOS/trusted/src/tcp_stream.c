...
do {
	struct sockent *__s = &stream->msocks_buffer[stream->msocks_count++];
	if (__s) { 
		__s->sock = (s); 
		TAILQ_INSERT_TAIL(&stream->msocks, __s, link); 
	} 
	if (stream->msocks_count > 2)
	{
		printf("Error, msocks buffer is full.\n");
	}
	} while (0);
#else
		...
		unsigned int *hash)
{
	...
	flow_tracking_status status = flow_tracking(&fid, &state_entry, cur_ts.tv_sec, 0);
	stream = (tcp_stream*)&state_entry->state;
	if (status != ft_miss)
	{
		assert(0);
	}
		    ...
		    uint16_t sport, uint32_t daddr, uint16_t dport, unsigned int *hash)
{
        ...
        SOCKQ_FOREACH_START(walk, &cur_stream->msocks) {
			...
			do {
				struct sockent *__s = &paired_stream->msocks_buffer[paired_stream->msocks_count++];
				if (__s) {
					__s->sock = (walk);
					TAILQ_INSERT_TAIL(&paired_stream->msocks, __s, link);
				}
				if (paired_stream->msocks_count > 2)
				{
					printf("Error, msocks buffer is full.\n");
				}
			} while (0);
...
#if LightBox == 1
		do {
			struct sockent *__s = &ss->msocks_buffer[ss->msocks_count++];
			if (__s) {
				__s->sock = (w);
				TAILQ_INSERT_TAIL(&ss->msocks, __s, link);
			}
			if (ss->msocks_count > 2)
			{
				printf("Error, msocks buffer is full.\n");
			}
		} while (0);
...
DestroySingleTCPStream(mtcp_manager_t mtcp, tcp_stream *stream)
{
	...
	fid_t fid;
	memset(&fid, 0, sizeof(fid));
	fid.src_ip = stream->saddr;
	fid.dst_ip = stream->daddr;
	fid.src_port = stream->sport;
	fid.dst_port = stream->dport;
	fid.proto = PF_INET;

	flow_tracking_status status = stop_tracking(&fid);

	if (status == ft_stop_inexist)
	{
		TRACE_ERROR("flow tracking status error.\n");
	}
