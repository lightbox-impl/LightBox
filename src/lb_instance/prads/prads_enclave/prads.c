...
void continue_parse(packetinfo *pi);
...
int filter_packet(const int af, void *ip);

/* F U N C T I O N S  ********************************************************/

uint16_t inline ntoh_short(uint16_t n) {
  return ((((unsigned short)(n) & 0xFF)) << 8) | 
         ((((unsigned short)(n) & 0xFF00)) >> 8);
}

uint32_t inline hton_long(uint32_t n) {
...
uint32_t inline ntoh_long(uint32_t n) {
  return  hton_long(n);
}
...
void got_packet(u_char * useless, const pcap_pkthdr *pheader, const u_char * packet)
{
    ...
    inpacket = 1;
#if CAIDA==1
    // In CAIDA Internet trace all L2 headers are stripped
    ...
    if (pi->eth_type == ETHERNET_TYPE_IP) {
        ...
        // skip for this moment
        //prepare_ip6(pi);
        //parse_ip6(pi);
#if SGBOX==1
        ferry_pkt_status[ferry_idx] = ferry_pkt_invalid;
#endif
        goto packet_end;
    ...
    switch (af) {
        ...
        case AF_INET6:
        {
            ...
            for (i = 0; i < MAX_NETS && i < nets; i++) {
                ...
                IP6ADDR0(&tmp_netmask) = hton_long(IP6ADDR0(&network[i].mask));
                IP6ADDR1(&tmp_netmask) = hton_long(IP6ADDR1(&network[i].mask));
                IP6ADDR2(&tmp_netmask) = hton_long(IP6ADDR2(&network[i].mask));
                IP6ADDR3(&tmp_netmask) = hton_long(IP6ADDR3(&network[i].mask));
...
void prepare_eth (packetinfo *pi)
{
    ...
    pi->eth_type = ntoh_short(pi->eth_hdr->eth_ip_type);
...
void check_vlan (packetinfo *pi)
{
    if (pi->eth_type == ETHERNET_TYPE_8021Q) {
        ...
        pi->eth_type = ntoh_short(pi->eth_hdr->eth_8_ip_type);
        pi->eth_hlen += 4;
    ...
    /* This is b0rked - kwy and ebf fix */
        ...
        pi->eth_type = ntoh_short(pi->eth_hdr->eth_82_ip_type);
        pi->eth_hlen += 8;
    }
    return;
}
...
void prepare_ip4 (packetinfo *pi)
{
    ...
    pi->packet_bytes = (ntoh_short(pi->ip4->ip_len) - (IP_HL(pi->ip4) * 4));
...
inline int sgbox_proto_unsupport(packetinfo *pi) {
    uint8_t proto = pi->ip4->ip_p;
    return (proto == IP_PROTO_IP4 ||
            proto == IP_PROTO_IP6 ||
            proto == IP_PROTO_GRE);
}

#if SGBOX==1 && NO_BUNDLE==0
void parse_ip4 (packetinfo *pi)
{
    switch (pi->ip4->ip_p) {
        case IP_PROTO_TCP:
...
// postpone parsing until the entire ferry is prepared/tracked
        ...
        case GRE_VERSION_0:
            ...
            if (GRE_FLAG_ISSET_ROUTE(pi->greh))
            {
                ...
                while (1)
                {
                    ...
                    if (gsre != NULL && (ntoh_short(gsre->af) == 0) && (gsre->sre_length == 0))
                        break;
...
void prepare_ip6 (packetinfo *pi)
{
    ...
    pi->packet_bytes = ntoh_short(pi->ip6->len);
...
void parse_udp (packetinfo *pi)
{
    ...
    if ( ntoh_short(pi->s_port) == 53 ||  ntoh_short(pi->s_port) == 5353 ) {
    ...
    if (IS_COSET(config, CO_DHCP) && ntoh_short(pi->s_port) == 68 && ntoh_short(pi->d_port) == 67) {
        dhcp_fingerprint(pi); /* basic DHCP parsing*/
    }
    // if (IS_COSET(config,CO_DNS) && (pi->sc == SC_SERVER && ntoh_short(pi->s_port) == 53)) passive_dns (pi);
...
void udp_guess_direction(packetinfo *pi)
{
    ...
    if (ntoh_short(pi->d_port) == 53) { 
        if (pi->sc == SC_CLIENT) return;
            else pi->sc = SC_CLIENT;

    } else if (ntoh_short(pi->s_port) == 53) {
...
void check_interrupt()
{
    ...
    //    intr_flag = 0;
    //}
    eprintf("[*] Interrupted with code %d!\n", intr_flag);
    intr_flag = 0;
    /*end_sessions();
    clear_asset_list();
    end_all_sessions();*/
...
//
//    vlog(2, "%08x =? %08x, endianness: %s\n\n", 0xdeadbeef, ntohl(0xdeadbeef), (0xdead == ntoh_short(0xdead)?"big":"little") );
//
