...
#if LightBox==1
        new_oa = (os_asset *)mem_util_calloc(sizeof(os_asset));
#else
...
#if LightBox==1
        new_sa = (serv_asset *)mem_util_calloc(sizeof(serv_asset));
#else
...
void add_asset(packetinfo *pi)
{
    ...
    config->pr_s.assets++;
#if LightBox==1
    masset = (asset *)mem_util_calloc(sizeof(asset));
#else
