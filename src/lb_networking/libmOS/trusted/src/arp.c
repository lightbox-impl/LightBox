...
unsigned char *GetDestinationHWaddr(uint32_t dip) {
	...
	static unsigned char none_haddr[ETH_ALEN] = {
	    0,
	};
	return none_haddr;
#endif
	...
	for (i = 0; i < g_config.mos->arp_table->num; i++) {
		if (g_config.mos->arp_table->ent[i]->prefix == 1) {
			if (g_config.mos->arp_table->ent[i]->ip == dip) {
				d_haddr =
				    g_config.mos->arp_table->ent[i]->haddr;
				break;
			}
		} else {
			if ((dip & g_config.mos->arp_table->ent[i]->mask) ==
			    g_config.mos->arp_table->ent[i]->masked_ip) {
				if (g_config.mos->arp_table->ent[i]->prefix >
				    prefix) {
					d_haddr =
					    g_config.mos->arp_table->ent[i]
						->haddr;
					prefix = g_config.mos->arp_table->ent[i]
						     ->prefix;
				}
			}
		}
...
/*----------------------------------------------------------------------------*/
static int ARPOutput(struct mtcp_manager *mtcp, int nif, int opcode,
	...
	struct arphdr *arph = (struct arphdr *)EthernetOutput(
	    mtcp, NULL, ETH_P_ARP, nif, dst_haddr, sizeof(struct arphdr), 0);
	if (!arph) {
		return -1;
	}
	...
	memcpy(arph->ar_sha, g_config.mos->netdev_table->ent[nif]->haddr,
	       arph->ar_hln);
	if (target_haddr) {
...
int RegisterARPEntry(uint32_t ip, const unsigned char *haddr) {
	...
	g_config.mos->arp_table->ent[idx] =
	    calloc(1, sizeof(struct _arp_entry));
	if (!g_config.mos->arp_table->ent[idx]) exit(0);
...
void RequestARP(mtcp_manager_t mtcp, uint32_t ip, int nif, uint32_t cur_ts) {
	...
	ent =
	    (struct arp_queue_entry *)calloc(1, sizeof(struct arp_queue_entry));
	if (ent == NULL) {
...
/*----------------------------------------------------------------------------*/
static int ProcessARPRequest(mtcp_manager_t mtcp, struct arphdr *arph, int nif,
			     uint32_t cur_ts) {
	unsigned char *temp;
...
/*----------------------------------------------------------------------------*/
static int ProcessARPReply(mtcp_manager_t mtcp, struct arphdr *arph,
			   uint32_t cur_ts) {
	unsigned char *temp;
	struct arp_queue_entry *ent;
...
int ProcessARPPacket(mtcp_manager_t mtcp, uint32_t cur_ts, const int ifidx,
	...
	struct arphdr *arph =
	    (struct arphdr *)(pkt_data + sizeof(struct ethhdr));
	int i;
	int to_me = FALSE;
	...
	for (i = 0; i < g_config.mos->netdev_table->num; i++) {
		if (arph->ar_tip ==
		    g_config.mos->netdev_table->ent[i]->ip_addr) {
			to_me = TRUE;
		}
	}

	if (!to_me) return TRUE;
...
/*----------------------------------------------------------------------------*/
// Publish my address
void PublishARP(mtcp_manager_t mtcp) {
	int i;
	for (i = 0; i < g_config.mos->netdev_table->num; i++) {
		ARPOutput(mtcp, g_config.mos->netdev_table->ent[i]->ifindex,
			  arp_op_request, 0, NULL, NULL);
	}
}
/*----------------------------------------------------------------------------*/
...
void PrintARPTable() {
	...
	if (g_config.mos->arp_table->num == 0) TRACE_CONFIG("(blank)\n");
...
/*----------------------------------------------------------------------------*/
void ForwardARPPacket(struct mtcp_manager *mtcp, struct pkt_ctx *pctx) {
	unsigned char *haddr;

	if (g_config.mos->nic_forward_table != NULL) {
		pctx->out_ifidx = g_config.mos->nic_forward_table
				      ->nic_fwd_table[pctx->p.in_ifidx];
		if (pctx->out_ifidx != -1) {
			haddr = pctx->p.ethh->h_dest;
			struct arphdr *arph = (struct arphdr *)EthernetOutput(
			    mtcp, NULL, ETH_P_ARP, pctx->out_ifidx, haddr,
