...
void* etap_thread(void* tput) {
    sgx_status_t ret = SGX_ERROR_UNEXPECTED;
    ret = ecall_etap_start(global_eid, (double*)tput, etap_args.record_size,
...
void* middlebox_thread(void* useless) {
    sgx_status_t ret = ecall_mb(global_eid);
    if (ret != SGX_SUCCESS) {
...
extern "C" {
void ocall_print_string2(const char* buf) { printf("%s", buf); }
}
...
int SGX_CDECL main(int argc, char* argv[]) {
	...
	double tput2;
	rc = pthread_create(&etap_send_trd, NULL, etap_send_thread, &tput2);
	if (rc) {
	    printf("Fail to create thread %lu\n!", etap_send_trd);
	    exit(1);
	}
	...
	rc = pthread_join(etap_send_trd, NULL);
	if (rc) {
	    printf("Fail to join thread %lu\n!", etap_send_trd);
	    exit(1);
	}
