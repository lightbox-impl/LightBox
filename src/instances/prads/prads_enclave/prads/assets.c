...
#if SGBOX==1 || LightBox==1
#include "../sg-box/sgbox_utils_t.h"
#endif
...
#if SGBOX==1 || LightBox==1
        new_oa = (os_asset *)sgbox_calloc(sizeof(os_asset));
#else
...
#if SGBOX==1 || LightBox==1
        new_sa = (serv_asset *)sgbox_calloc(sizeof(serv_asset));
#else
...
void add_asset(packetinfo *pi)
{
    ...
    config->pr_s.assets++;
#if SGBOX==1 || LightBox==1
    masset = (asset *)sgbox_calloc(sizeof(asset));
#else
