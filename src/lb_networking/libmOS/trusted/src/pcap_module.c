...
/*----------------------------------------------------------------------------*/
void pcap_init_handle(struct mtcp_thread_context* ctxt) {
	ctxt->io_private_context = &g_pcap_ctx;
}
/*----------------------------------------------------------------------------*/
int32_t pcap_recv_pkts(struct mtcp_thread_context* ctxt, int ifidx) {
	if (ifidx < 0 || ifidx >= MAX_DEVICES) return -1;
	...
	if (ppc->phdr.caplen > 1514) {
		return 0;
	}

	/* sanity check */
	if (ppc->rdata == NULL) return 0;
...
/*----------------------------------------------------------------------------*/
uint8_t* pcap_get_rptr(struct mtcp_thread_context* ctxt, int ifidx, int index,
		       uint16_t* len) {
	struct pcap_private_context* ppc = ctxt->io_private_context;
	if (ppc->rdata == NULL) return NULL;

	*len = ppc->phdr.caplen;
	return (uint8_t*)ppc->rdata;
}
/*----------------------------------------------------------------------------*/
int pcap_send_pkts(struct mtcp_thread_context* ctxt, int nif) {
	int ret = 0;
	struct pcap_private_context* ppc = ctxt->io_private_context;
...
/*----------------------------------------------------------------------------*/
uint8_t* pcap_get_wptr(struct mtcp_thread_context* ctxt, int ifidx,
		       uint16_t pktsize) {
	struct pcap_private_context* ppc = ctxt->io_private_context;
	/* phdr can be reused */
...
/*----------------------------------------------------------------------------*/
int pcap_get_nif(struct ifreq* ifr) {
	int i;

	for (i = 0; i < g_config.mos->netdev_table->num; i++)
		if (!strcmp(ifr->ifr_name, g_pcap_ctx.dev_name[i])) return i;

	return -1;
}
/*----------------------------------------------------------------------------*/
int32_t pcap_select(struct mtcp_thread_context* ctxt) { return 0; }
/*----------------------------------------------------------------------------*/
void pcap_destroy_handle(struct mtcp_thread_context* ctxt) {
	int i;
	struct pcap_private_context* ppc = ctxt->io_private_context;
	if (!ppc) return;
...
/*----------------------------------------------------------------------------*/
void pcap_load_module_upper_half(void) {
	int i;
	...
	for (i = 0; i < g_config.mos->netdev_table->num; i++) {
...
#endif
		// printf("connect to pcap interface:%s\n", ent[i]->dev_name);
		...
		if (pcap_set_buffer_size(g_pcap_ctx.handle[i],
					 PCAP_BUFFER_SIZE) != 0) {
