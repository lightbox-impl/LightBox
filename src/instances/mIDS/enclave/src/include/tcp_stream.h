...
struct tcp_send_vars
{
	...
	unsigned char d_haddr[16];	/* cached destination MAC address */
	...
	struct tcp_send_buffer sndbuf_buffer;
...
#if LightBox == 1
	struct sockent msocks_buffer[2];
	int msocks_count;

	//struct socket_map msocks_map_buffer[2];
	//int msocks_map_count;

	//struct mon_stream msocks_stream_buffer[2];
	//int msock_stream_count;
#endif
