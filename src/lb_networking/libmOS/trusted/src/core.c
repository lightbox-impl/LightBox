...
void RunMainLoop(struct mtcp_thread_context *ctx)
{
	...
	// for sgx single thread 
	// first set context, when secend call, enter main loop
	if (ctx)
	{
		saved_ctx = ctx;
		return;
	...
	while ((!ctx->done || mtcp->flow_cnt) && !ctx->exit) 
	{
		...
		{
			printf("None of net devices, exit.\n");
			return;
		}
		...
		//static struct etime beg = { 0,0 };
		//static struct etime end = { 0,0 };
		//if (!beg.s)
		//{
		//	ocall_get_time2((int*)&beg.s, (int*)&beg.ns);
		//	ocall_get_time2((int*)&end.s, (int*)&end.ns);
		//}
		//if (++mtcp->ctx_pkt_count >= printPktLoop)
		//{
		...
		//	ocall_get_time2((int*)&end.s, (int*)&end.ns);
		//	totalCount += mtcp->ctx_pkt_count;

		//	snprintf(str, 128, "Total,%lld,Delay,%d,us gap is,%d\n", totalCount, diffTime(&beg, &end), mtcp->ctx_pkt_count);

		//	printf(str);
		//	//ocall_write_file(fileName, str, strlen(str), 1);
		//	beg = end;
		//	mtcp->ctx_pkt_count = 0;
		//}
...
InitializeMTCPManager(struct mtcp_thread_context* ctx)
{
	...
	posix_seq_srand((unsigned)ctx->cpu);
...
#define	IS_HUGEPAGE 0
...
#if LightBox == 1
	// the mem is move to stream
	int non_count = 2;
	// the count is based application
	int few_count = 32;
	// the count is based throughtput
	int normal_count = g_config.mos->max_concurrency;
	// full count 
	int full_count = g_config.mos->max_concurrency;
	printf("full_count is %d", g_config.mos->max_concurrency);
#else
	int non_count = g_config.mos->max_concurrency;
	int few_count = g_config.mos->max_concurrency;
	int normal_count = g_config.mos->max_concurrency;
	int full_count = g_config.mos->max_concurrency;
	...
	if (!(mtcp->sockent_pool = MPCreate(sizeof(struct sockent),
			sizeof(struct sockent) * non_count * 3, 0))) {
	...
	mtcp->smap = (socket_map_t)calloc(few_count, sizeof(struct socket_map));
	if (!mtcp->smap) {
		perror("calloc");
	...
	if (mon_app_exists) {
		mtcp->msmap = (socket_map_t)calloc(full_count, sizeof(struct socket_map));
		if (!mtcp->msmap) {
			perror("calloc");
	...
	mtcp->sendq = CreateStreamQueue(few_count);
	if (!mtcp->sendq) {
		CTRACE_ERROR("Failed to create send queue.\n");
		return NULL;
	}
	mtcp->ackq = CreateStreamQueue(few_count);
	if (!mtcp->ackq) {
		CTRACE_ERROR("Failed to create ack queue.\n");
		return NULL;
	}
	mtcp->closeq = CreateStreamQueue(few_count);
	if (!mtcp->closeq) {
		CTRACE_ERROR("Failed to create close queue.\n");
		return NULL;
	}
	mtcp->closeq_int = CreateInternalStreamQueue(few_count);
	if (!mtcp->closeq_int) {
		CTRACE_ERROR("Failed to create close queue.\n");
		return NULL;
	}
	mtcp->resetq = CreateStreamQueue(few_count);
	if (!mtcp->resetq) {
		CTRACE_ERROR("Failed to create reset queue.\n");
		return NULL;
	}
	mtcp->resetq_int = CreateInternalStreamQueue(few_count);
	if (!mtcp->resetq_int) {
		CTRACE_ERROR("Failed to create reset queue.\n");
		return NULL;
	}
	mtcp->destroyq = CreateStreamQueue(few_count);
	if (!mtcp->destroyq) {
