...
static int findEmptyLoc(long long* dic)
{
	int i = 0;
	for (i = 0; i < 64; i++)
	{
		if ((*dic & (1 << i)) == 0)
		{
			//printf("find empty loc is %d.\n", i);
			*dic |= (1 << i);
			return i;
		}
	}
	//printf("can`t find loc is %lld.\n", *dic);
	return -1;
}
...
buf_first(tcprb_t *rb)
{
	...
	TRACE_ERROR("ALLOW_PKT_DROP use own buffer.\n");
	...
	if (rb->current_seg_no == MAX_SEG_COUNT)
	{
		TRACE_ERROR("tcpbufseg_buffer is full.\n");
		rb->seg_used = 0;
	}

	rb->current_seg_no++;
	tmp = &rb->tcpbufseg_buffer[findEmptyLoc(&rb->seg_used)];
...
buf_try_resize(tcprb_t *rb, int len, loff_t data, int datalen)
{
	...
	if (segdiff > 0) {
		...
		if (head > tail && headseg == tailseg) {
			...
			rb->current_seg_no++;
			tcpbufseg_t *seg = &rb->tcpbufseg_buffer[findEmptyLoc(&rb->seg_used)];
...
seq2loff(tcprb_t *rb, uint32_t seq, uint32_t isn)
{
	...
	//printf("seq2loff: off(%d) = seq(%d) - isn(%d)\n", off, seq, isn);
...
tcprb_cflen(tcprb_t *rb)
{
	...
	if (!cf || !cf->tail || (cf->head != rb->head))
	{
		//TRACE_ERROR("find un sequence pkt.\n");
...
tcprb_ffhead(tcprb_t *rb, int len)
{
	...
	if (cflen == ff) {
		...
		int __loc = (cf - rb->tcpfrag_t_buffer) / sizeof(rb->tcpfrag_t_buffer[0]);
		rb->frag_used &= ~(1 << __loc);
		rb->current_frag_no--;
#else
		TAILQ_REMOVE(&rb->frags, cf, link);
		frags_del(cf);
...
tcprb_resize_meta(tcprb_t *rb, int len)
{
	...
	for (fwalk = TAILQ_FIRST(&rb->frags); fwalk; fwalk = fnext) {
		...
		int __loc = (fwalk - rb->tcpfrag_t_buffer) / sizeof(rb->tcpfrag_t_buffer[0]);
		rb->frag_used &= ~(1 << __loc);
		rb->current_frag_no--;
#else
		TAILQ_REMOVE(&rb->frags, fwalk, link);
...
tcprb_resize(tcprb_t *rb, int len)
{
	...
	rb->current_seg_no = 0;
	rb->current_frag_no = 0;
	rb->seg_used = 0;
	rb->frag_used = 0;
	memset(rb->tcpbufseg_buffer, 0, sizeof(rb->tcpbufseg_buffer));
	memset(rb->tcpfrag_t_buffer, 0, sizeof(rb->tcpfrag_t_buffer));
#else
...
#if LightBox == 1 
						int __loc = (fnext - rb->tcpfrag_t_buffer) / sizeof(rb->tcpfrag_t_buffer[0]);
						rb->frag_used &= ~(1 << __loc);
						rb->current_frag_no--;
#else
						frags_del(fnext);
