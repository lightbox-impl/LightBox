	...
	if (n >= 0 && n < TIMERCOUNT) {
		...
		if (1) {
			if (debugTimers[n - 1].s != 0)
				debugTime[n] += diffTime(&debugTimers[n - 1],
							 &debugTimers[n]);
		} else {
...
void debugPin(int n) { ++pinCounter[n]; }
#else
...
void thread_printf(mtcp_manager_t mtcp, FILE *f_idx, const char *_Format, ...) {
	...
	//}
	//
	// len = vsnprintf(wbuf->buff + wbuf->buff_len, PRINT_LIMIT, _Format,
	// argptr); wbuf->buff_len += len;
	// pthread_mutex_unlock(&mtcp->logger->mutex);

	// va_end(argptr);
}
/*----------------------------------------------------------------------------*/
void DumpPacket(mtcp_manager_t mtcp, char *buf, int len, char *step,
		int ifindex) {
	//	struct ethhdr *ethh;
	//	struct iphdr *iph;
	//	struct udphdr *udph;
	...
	//
	//	if (ifindex >= 0)
	//		thread_printf(mtcp, mtcp->log_fp, "%s %d %u", step,
	//ifindex, mtcp->cur_ts); 	else 		thread_printf(mtcp, mtcp->log_fp, "%s ?
	//%u", step, mtcp->cur_ts);
	//
	...
	//				ethh->h_dest[5]);
	//
	//		thread_printf(mtcp, mtcp->log_fp, "protocol %04hx  ",
	//ntohs(ethh->h_proto)); 		goto done;
	//	}
	//
	//	thread_printf(mtcp, mtcp->log_fp, " ");
	//
	...
	//
	//	t = (uint8_t *)&iph->saddr;
	//	thread_printf(mtcp, mtcp->log_fp, "%u.%u.%u.%u", t[0], t[1],
	//t[2], t[3]); 	if (iph->protocol == IPPROTO_TCP || iph->protocol ==
	//IPPROTO_UDP) 		thread_printf(mtcp, mtcp->log_fp, "(%d)",
	//ntohs(udph->source));
	//
	//	thread_printf(mtcp, mtcp->log_fp, " -> ");
	//
	...
	//		thread_printf(mtcp, mtcp->log_fp, "(bad checksum %04x
	//should be %04x) ", 				ntohs(org_csum), ntohs(correct_csum)); 		iph->check =
	//org_csum;
	//	}
	//
	//	switch (iph->protocol) {
	...
	//
	//		thread_printf(mtcp, mtcp->log_fp, "seq %u ",
	//ntohl(tcph->seq)); 		if (tcph->ack) 			thread_printf(mtcp, mtcp->log_fp,
	//"ack %u ", ntohl(tcph->ack_seq)); 		thread_printf(mtcp, mtcp->log_fp,
	//"WDW=%u ", ntohs(tcph->window)); 		break; 	case IPPROTO_UDP:
	//		thread_printf(mtcp, mtcp->log_fp, "UDP ");
	//		break;
	//	default:
	//		thread_printf(mtcp, mtcp->log_fp, "protocol %d ",
	//iph->protocol); 		goto done;
	//	}
	// done:
...
void DumpIPPacketToFile(FILE *fout, const struct iphdr *iph, int len) {
	...
	//	if (iph->protocol == IPPROTO_TCP || iph->protocol ==
	//IPPROTO_UDP) 		fprintf(fout, "(%d)", ntohs(udph->source));
	//
	//	fprintf(fout, " -> ");
	//
	//	t = (uint8_t *)&iph->daddr;
	//	fprintf(fout, "%u.%u.%u.%u", t[0], t[1], t[2], t[3]);
	//	if (iph->protocol == IPPROTO_TCP || iph->protocol ==
	//IPPROTO_UDP) 		fprintf(fout, "(%d)", ntohs(udph->dest));
	//
