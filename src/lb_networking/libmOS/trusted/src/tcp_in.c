				...
				struct pkt_ctx *pctx)
{
	...
	*/
	...
	if (!rcvvar->rcvbuf) {
		...
		tcprb_t *rb;
		if (g_config.mos->rmem_size % UNITBUFSIZE || g_config.mos->rmem_size < 2)
			return NULL;

		if (!(rb = &rcvvar->rcvbuf_buffer))
			return NULL;
		...
		rb->lbufsegs = ((g_config.mos->rmem_size - 1) / UNITBUFSIZE) + 1;

		rb->buf_mgmt = cur_stream->buffer_mgmt;
		rb->mp = mtcp->bufseg_pool;
		rb->len = rb->metalen = g_config.mos->rmem_size;
		TAILQ_INIT(&rb->frags);
	...
	if (off >= 0) 
	{
		...
		if (HAS_STREAM_TYPE(cur_stream, MOS_SOCK_MONITOR_STREAM_ACTIVE)) 
		{
			...
			if (fflen > 0) {
				//printf("call MOS_ON_ERROR on buffer outrun.\n");
		...
		if (ret != 0)
		{
			//assert(pctx->p.eth_len < 1600);
			//cur_stream->recv_pkt_size += pctx->p.eth_len+24;
			cur_stream->recv_pkt_count += 1;
		}
		else
		{
			debugPin(1);
